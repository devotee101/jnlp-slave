...
RUN wget -q https://storage.googleapis.com/kubernetes-release/release/${KUBE_VERSION}/bin/linux/amd64/kubectl -O /usr/local/bin/kubectl && \
    ...
    apk add --no-cache sudo curl jq --repository http://dl-cdn.alpinelinux.org/alpine/v3.12/main

RUN apk upgrade --no-cache --repository http://dl-cdn.alpinelinux.org/alpine/v3.12/main

COPY jenkins.sudoers /etc/sudoers.d/
